...
accuracy.df <- data.frame(Model = seq(1, 7,1), Train_Accuracy_score = rep(0,7) ,Valid_Accuracy_score = rep(0,7))
```
...
#put in accuracy to data frame.
accuracy.df[4,3] <- confusionMatrix(bdtree.pred, mydata$Class)$overall[1]
...
accuracy.df[5,3] <- confusionMatrix(as.factor(ans), as.factor(mydata$Class))$overall[1]
...
accuracy.df[6,3] <- confusionMatrix(as.factor(bdrda2.pred$class), as.factor(mydata$Class))$overall[1]
```
...
as.data.frame(bdsvm.pred)[1]
as.data.frame(bdnvb.pred)[1]
as.data.frame(bdrda.pred)[1]

#Use median answer as output
ensamble.df <- cbind(as.data.frame(bdsvm.pred)[1],as.data.frame(bdnvb.pred)[1],as.data.frame(bdrda.pred)[1])
colnames(ensamble.df) <-c("svm", "nvb", "rda")
